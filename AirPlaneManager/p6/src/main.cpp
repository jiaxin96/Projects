...
void initialize(int &end_time,int &queue_limit,double &arrival_rate,double &departure_rate){
	...
	bool acceptable;
	do {
		cout << "Do you wang to monitor the Aircraft scheduling by yourself or system?[y/s]" << endl;
		cin >> _selection;
		if (_selection == "y" || _selection == "Y") {
			acceptable = true;
			is_format = true;
		}
		else if (_selection == "s" || _selection == "S") {
			acceptable = false;
			is_format = true;
		}
		if (is_format == false) cout << "Wrong _selectionion!Please _selection again" << endl;
	} while(!is_format);
	while (!acceptable) {
...
int main() {
	...
	for(int current_time=0;current_time<end_time;current_time++){
		...
		if (_selection == "Y" || _selection == "y") {
			do {
				cout << "In the " <<current_time << "s"<< ", how many planes do you want to arrive?" << flush;
				cin >> number_arrivals;
			} while(number_arrivals < 0);
		} else {
			number_arrivals= variable.possion(arrival_rate);
		}
		...
		if (_selection == "Y" || _selection == "y") {
			do {
				cout << "In the " <<current_time << "s"<< ", how many planes do you want to takeoff?" << flush;
				cin >> number_departures;
			} while(number_departures < 0);
		} else {
			number_departures=variable.possion(departure_rate);
		}
