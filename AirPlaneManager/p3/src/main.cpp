...
int main() {
	...
	for(int current_time=0;current_time<end_time;current_time++){
		...
		for(int i=0;i<number_arrivals;i++){
			...
			if (small_airport_takeoff.takeoffqueue_size() == 0 && small_airport_takeoff.landqueue_size() < small_airport_land.landqueue_size()) {
		...
		for(int j=0;j<number_departures;j++){
			...
			if(small_airport_takeoff.can_depart(current_plane)!=success) {
				if (small_airport_land.landqueue_size()) {
					if (small_airport_land.can_depart(current_plane) != success) {
						current_plane.refuse();
					}
				} else {
					current_plane.refuse();
