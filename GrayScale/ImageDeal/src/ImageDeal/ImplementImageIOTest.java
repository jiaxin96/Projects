...
public class ImplementImageIOTest {
	...
	public void testByteInfo() throws IOException {
		...
		byte[]  my = new byte[1024];
		byte[]  goal = new byte[1024];
		int myReadLength = myFs.read(my);
		int goalReadLength = goalFs.read(goal);
		while (-1 != myReadLength && -1 != goalReadLength) {
			assertArrayEquals(my, goal);
			myReadLength = myFs.read(my);
			goalReadLength = goalFs.read(goal);
		}
		myFs.close();
		goalFs.close();
		...
		myFs = new FileInputStream(image2_gray_mypath);
		goalFs = new FileInputStream(image2_graypath);
		myReadLength = myFs.read(my);
		goalReadLength = goalFs.read(goal);
		while (-1 != myReadLength && -1 != goalReadLength) {
			assertArrayEquals(my, goal);
		...
		myFs = new FileInputStream(image2_red_mypath);
		goalFs = new FileInputStream(image2_redpath);
		myReadLength = myFs.read(my);
		goalReadLength = goalFs.read(goal);
		while (-1 != myReadLength && -1 != goalReadLength) {
			assertArrayEquals(my, goal);
